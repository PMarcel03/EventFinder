...
async function fetchContent(url) {
  ...
  else //Use Axios for standard HTTP/HTTPS requests
  {
    try {
...
async function normalizeOrganizer(name) {
  if (!name) return null;
  try {
    const response = await axios.post(`${API_URL}/organizers/normalize`, {
      name,
    });
    return response.data._id;
  } catch (error) {
...
async function saveEvent(event) {
  try {
    ...
    //Log specific error message from the API if available
    const apiError = error.response ? error.response.data.error : error.message;
...
async function runScraper() {
  ...
  for (const event of events) {
    ...
    if (event.venueId && event.organizerId) {
      ...
      console.warn(
        `Skipping event "${event.title}": Normalization failed for Venue/Organizer.`,
      );
    }
